...
function CategoryScroll({ category, movies }) {
  const categoryMovies = movies.filter((m) => m.genre_ids.includes(category.id)).slice(0, 20);
        ...
        {categoryMovies.map((movie) => (
          <article className="card movie-poster" key={movie.id}>
            ...
            <div className="rating">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 115.53 110.12" className="star-icon">
                <polygon
                  className="starSvg"
